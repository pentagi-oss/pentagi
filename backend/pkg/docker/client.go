...
func (dc *dockerClient) CleanupAllResources(ctx context.Context) error {
	...
	purgeContainer := func(containerID string, dbID int64) {
		defer wg.Done()
		logger := logger.WithField("local_id", containerID)

		if err := dc.PurgeContainer(ctx, containerID, dbID); err != nil {
			logger.WithError(err).Errorf("failed to purge container")
		}
	...
	for _, flow := range flows {
		...
		default: // FlowStatusFinished, FlowStatusFailed
			for _, container := range flowContainersMap[flow.ID] {
				...
				case database.ContainerStatusStarting, database.ContainerStatusRunning:
					wg.Add(1)
					go purgeContainer(container.LocalID.String, container.ID)
				}
			}
		}
	}
